  ...
  <div class="doctor-table">
    ...
    <table v-if="doctors.length > 0">
      ...
      <tbody>
        <tr v-for="doctor in doctors" :key="doctor.id">
          ...
          <td class="actions">
            <button @click="$emit('edit-doctor', doctor)" class="btn-edit" title="Editar">✏️</button>
            <button @click="confirmDelete(doctor)" class="btn-delete" title="Eliminar">🗑️</button>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <div v-if="showDeleteModal" class="delete-modal">
      <div class="delete-content">
        ...
        <p>¿Estás seguro de eliminar a <strong>{{ doctorToDelete?.nombre }} {{ doctorToDelete?.apellido }}</strong>?</p>
        <div class="delete-buttons">
          <button @click="deleteDoctor" class="btn-confirm">Sí, eliminar</button>
...
export default {
  ...
  emits: ['add-doctor', 'edit-doctor', 'delete-doctor', 'update:errorMessage', 'doctor-deleted'],
  data() {
    return {
  ...
  methods: {
    ...
    async deleteDoctor() {
      try {
        await DoctorClient.deleteDoctor(this.doctorToDelete.id);
